...
/* ---------------------------------------------------------------------------------------------------------
Function: MSOT_geometry_fnc_getSafePos
...
Returns:  Position
Examples:
          _safePosArea = [(getMarkerPos "Town"),200,20] call MSOT_geometry_fnc_getSafePos;
          _safePosRandomBorder = [(getMarkerPos "Town"),200,20,true,-1] call MSOT_geometry_fnc_getSafePos;
...
If(_isborder)then
{
   ...
   _point = [_center,(_radius + _tolerance),_degrees] call BFUNC(relPos);
   _saPos = [_point,1,_tolerance,5,20,0] call BFUNC(findSafePos);
   If(count _saPos > 0)then
   {
     _slope_chk = [_saPos] call MFUNC(spawnhelp,getSlope);
   ...
   If(_deg_isRandom)then
   {
     while{count _output == 0 && _deg_count < 359}do
     {
       ...
       _saPos = [_center,1,(_radius + _tolerance),5,50,0] call BFUNC(findSafePos);
       If(count _saPos > 0)then
       {
         _slope_chk = [_saPos] call MFUNC(spawnhelp,getSlope);
         If(_slope_chk)then{_output = _saPos;};
