...
/* -----------------------------------------------------------------------------------------------
Function: MSOT_spawnhelp_fnc_checkHousePos
...
Examples:
          _pos_list = [_building] call MSOT_spawnhelp_fnc_checkHousePos;
                   ...
                   {_level_count pushBackUnique (ceil(_x select 2));}forEach _inhousepos;
                    ...
                    _last_levelno = -1;
                    {
                      ...
                      for "_i" from 0 to ((count _inhousepos) - 1) do
                      {
                        ...
                        {
                          ARR_ADDVAR(_add_arr,_h_pos);
                        };
                      };
                      _last_levelno = _x;
                      If(count _add_arr > 0)then{ARR_ADDVAR(_output,_add_arr);};
